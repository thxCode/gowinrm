...
var _ = Describe("NtlmSsp", func() {
	...
	Context("TLS but not check", func() {
		It("powershell call \"ipconfig.exe /all\"", func() {
			...
			ssp := gowinrm.NewNtlmSSP(username, password, hostname, gowinrm.NewSecurity().WithoutVerify())
	...
	Context("TLS", func() {
		It("powershell call \"dir c:\\\"", func() {
			...
			ssp := gowinrm.NewNtlmSSP(username, password, hostname, gowinrm.NewSecurity().WithServerCAs(serverPemCerts))
			session := gowinrm.NewSession(ssp)
			defer session.Close()
